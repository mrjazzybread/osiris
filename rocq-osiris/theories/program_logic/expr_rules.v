...
Section ewp_binding_rules.
  ...
  Lemma ewp_eval_bindings_cons_total η p e bs
    ...
    EWP[ι] eval η e @ E <|Ψ|> {{ ensures v, φ1 v }} -∗
    EWP[ι] eval_bindings η bs @ E <|Ψ|> {{ ensures v, φ v }} -∗
    (∀ v δ, φ1 v -∗ φ δ -∗ ⌜pure_wp (irrefutably_extend η δ p v) (φs v) ⊥⌝) -∗
    (∀ v δ, φ1 v -∗  ⌜φs v δ⌝ -∗ φ' δ) -∗
    EWP[ι] eval_bindings η (Binding p e :: bs) @ E <|Ψ|> {{ ensures v, φ' v }}.
  Proof.
  ...
  Lemma ewp_eval_bindings_cons η p e bs φ1 φs φ E Ψ :
    EWP[ι] eval η e @ E <|Ψ|> {{ φ1 }} -∗
    EWP[ι] eval_bindings η bs @ E <|Ψ|> {{ φs }} -∗
    (∀ v : exn, φ1 (O2Throw v) -∗ φ (O2Throw v)) -∗
    (∀ v : exn, φs (O2Throw v) -∗ φ (O2Throw v)) -∗
    (∀ v δ, φ1 (O2Ret v) ∗ φs (O2Ret δ) -∗
      EWP[ι] widen (irrefutably_extend η δ p v)  @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι] eval_bindings η (Binding p e :: bs) @ E <|Ψ|> {{ φ }}.
  Proof.
...
Section ewp_rules_expr.
  ...
  Lemma ewp_EApp_exn η e1 e2 φ1 φ2 φ E Ψ :
    EWP[ι] eval η e1 @ E <|Ψ|> {{ φ1 }} -∗
    EWP[ι] eval η e2 @ E <|Ψ|> {{ φ2 }} -∗
    (∀ v, φ1 (O2Throw v) -∗ φ (O2Throw v)) -∗
    (∀ v, φ2 (O2Throw v) -∗ φ (O2Throw v)) -∗
    (∀ v1 v2, φ1 (O2Ret v1) -∗ φ2 (O2Ret v2) -∗ EWP[ι] call v1 v2 @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι] eval η (EApp e1 e2) @ E <|Ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_EApp' η e1 e2 φ1 φ2 φ E Ψ :
    EWP[ι] eval η e1 @ E <|Ψ|> {{ ensures v, φ1 v }} -∗
    EWP[ι] eval η e2 @ E <|Ψ|> {{ ensures v, φ2 v }} -∗
    (∀ v1 v2, φ1 v1 -∗ φ2 v2 -∗
       EWP[ι] call v1 v2 @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι] eval η (EApp e1 e2) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H1 H2 H".
  ...
  Lemma ewp_call_nonrec η x_arg body v_arg φ E Ψ:
    ▷EWP[ι] eval ((x_arg, v_arg) :: η) body @ E <|Ψ|> {{ φ }} -∗
    EWP[ι] call (VClo η (AnonFun x_arg body)) v_arg @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H".
  ...
  Lemma ewp_call_rec η f x body bs v φ E Ψ:
    ...
    ▷ EWP[ι] eval ((x, v) :: eval_rec_bindings η bs ++ η) body @ E <|Ψ|> {{ φ }} -∗
    EWP[ι] call (VCloRec η bs f) v @ E <|Ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_call_rec_1 η f x body v φ E Ψ:
    ...
    ▷EWP[ι] eval ((x, v) :: (f, vf) :: η) body @ E <|Ψ|> {{ φ }} -∗
    EWP[ι] call vf v @ E <|Ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_evals η E es φs φe ψ:
    ([∗ list] ei;φi ∈ es;φs,
       EWP[ι] eval η ei @ E <| ψ |> {{ RET v => φi v | EXN e => φe e }}) -∗
    EWP[ι] evals η es @ E <| ψ |>
  ...
  Lemma ewp_ETuple_forward_exn η es φs φe E Ψ :
    ([∗ list] ei; φi ∈ es; φs, EWP[ι] eval η ei @ E <|Ψ|> {{ RET v => φi v | EXN e => φe e }}) -∗
    EWP[ι] eval η (ETuple es) @ E <|Ψ|>
      {{  RET v => ∃ vs, ⌜v = VTuple vs⌝ ∗ [∗ list] vi; φi ∈ vs; φs, φi vi
        | EXN e => φe e }}.
  Proof.
    iIntros "H /=". simpl_eval.
    iApply ewp_bind'.
    iApply (ewp_mono _ _ _ (  RET vs => [∗ list] vi; φi ∈ vs; φs, φi vi
                          | EXN v => φe v) with "[-]")%I.
  ...
  Lemma ewp_ETuple_forward η es φs E Ψ :
    ([∗ list] ei; φi ∈ es; φs, EWP[ι] eval η ei @ E <|Ψ|> {{ ensures v, φi v }}) -∗
    EWP[ι] eval η (ETuple es) @ E <|Ψ|>
  ...
  Lemma ewp_EPair_exn η e1 e2 (φ1 φ2 φ : outcome2 val exn -> _) E Ψ :
    ...
    EWP[ι] eval η (EPair e1 e2) @ E <|Ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_EPair_ret η e1 e2 φ1 φ2 E Ψ φ :
    ...
    EWP[ι] eval η (EPair e1 e2) @ E <|Ψ|> {{ ensures v, φ v }}.
  Proof.
    iIntros "H1 H2 P".
  ...
  Lemma ewp_EPair_ret_encode η e1 e2 `{Encode A} `{Encode B} (v1 : A) (v2 : B) R1 R2 E Ψ :
    EWP[ι] eval η e1 @ E <|Ψ|> {{ ensures #r, ⌜r = v1⌝ ∗ R1 }} -∗
    EWP[ι] eval η e2 @ E <|Ψ|> {{ ensures #r, ⌜r = v2⌝ ∗ R2 }} -∗
    EWP[ι] eval η (EPair e1 e2) @ E <|Ψ|> {{ ensures #r, ⌜r = (v1, v2)⌝ ∗ R1 ∗ R2 }}.
  Proof.
    iIntros "H1 H2".
  ...
  Lemma ewp_EPair_forward η e1 e2 `{Encode A} `{Encode B} (v1 : A) (v2 : B) R1 R2 E Ψ :
    EWP[ι] eval η e1 @ E <|Ψ|> {{ RET= #v1, R1 }} -∗
    EWP[ι] eval η e2 @ E <|Ψ|> {{ RET= #v2, R2 }} -∗
    EWP[ι] eval η (EPair e1 e2) @ E <|Ψ|> {{ RET= #(v1, v2), R1 ∗ R2 }}.
  Proof.
    iIntros "H1 H2".
  ...
  Lemma ewp_EData η c es E ψ φs φ :
    ...
    EWP[ι] eval η (EData c es) @ E <|ψ|> {{ ensures v, φ v }}.
  Proof.
    iIntros "Hes Hmon /=".
    simpl_eval.
    iApply ewp_bind.
  ...
  Lemma ewp_EConstant η c E Ψ φ :
    φ (O2Ret (VConstant c)) -∗
    EWP[ι] eval η (EConstant c) @ E <|Ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_EXData_exn η π l e E ψ φ1 φ :
    ...
    EWP[ι] evals η e @ E <|ψ|> {{ φ1 }} -∗
    propagate_exn φ1 φ -∗
    propagate_ret_fmap (λ v : list val, VXData l v) φ1 φ -∗
    EWP[ι] eval η (EXData π e) @ E <|ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_EXData η π l es E ψ φs φ :
    ...
    EWP[ι]eval η (EXData π es) @ E <|ψ|> {{ ensures v, φ v }}.
  Proof.
  ...
  Proof.
    ...
    - iApply ewp_bind. iApply (ewp_mono with "H1").
      iIntros ([|]); [ | iIntros ([]) ].
      iIntros "(%n & -> & H)".
      iApply ewp_ret. iExists _. by iFrame.
    - iApply ewp_bind. iApply (ewp_mono with "H2").
      iIntros ([|]); [ | iIntros ([]) ].
      iIntros "(%n & -> & H)".
      iApply ewp_ret. iExists _. by iFrame.
    - iIntros (i1 i2) "(H1 & H2)".
  ...
  Lemma ewp_EIntAdd_simple η e1 e2 (n1 n2 : Z) (R1 R2 : iProp Σ) E Ψ :
    EWP[ι] eval η e1 @ E <|Ψ|> {{ RET= #n1, R1 }} -∗
    EWP[ι] eval η e2 @ E <|Ψ|> {{ RET= #n2, R2 }} -∗
    EWP[ι] eval η (EIntAdd e1 e2) @ E <|Ψ|> {{ RET= #(n1 + n2)%Z, R1 ∗ R2 }}.
  Proof.
    iIntros "H1 H2".
  ...
  Lemma ewp_EOpEq_exn η e1 e2 φ1 φ2 φ E Ψ :
    ...
    (∀ v1 v2, φ1 (O2Ret v1) ∗ φ2 (O2Ret v2) -∗
      EWP[ι]eq_val v1 v2 @ E <|Ψ|> {{ ensures b, φ (O2Ret (VBool b)) }}) -∗
    EWP[ι]eval η (EOpEq e1 e2) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H1 H2 E1 E2 P /=". simpl_eval.
    iApply (ewp_Par with "H1 H2 [E1] [E2]").
    - iIntros (v) "H /=". iApply ewp_throw. iApply ("E1" with "H").
  ...
  Lemma ewp_EOpEq_simple_Z η e1 e2 (n1 n2 : Z) (R1 R2 : iProp Σ) E Ψ :
    ...
    EWP[ι]eval η e1 @ E <|Ψ|> {{ RET= #n1, R1 }} -∗
    EWP[ι]eval η e2 @ E <|Ψ|> {{ RET= #n2, R2 }} -∗
    EWP[ι]eval η (EOpEq e1 e2) @ E <|Ψ|> {{ RET= #(n1 =? n2)%Z, R1 ∗ R2 }}.
  Proof.
  ...
  Lemma ewp_ELet_exn η bs e φ1 φ E Ψ :
    EWP[ι] eval_bindings η bs @ E <|Ψ|> {{ φ1 }} -∗
    (∀ v, φ1 (O2Throw v) -∗ φ (O2Throw v)) -∗
    (∀ δ, φ1 (O2Ret δ) -∗ EWP[ι] eval (δ ++ η) e @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι] eval η (ELet bs e) @ E <|Ψ|> {{ φ }}.
  Proof.
  ...
  Corollary ewp_ELet_singleton_total {η p e e'} (Φ : val -> iProp Σ) φ φ' E Ψ:
    ...
    (∀ v δ, Φ v -∗ ⌜φ v δ⌝ -∗
            EWP[ι] eval (δ ++ η) e @ E <|Ψ|> {{ φ' }}) -∗
    EWP[ι] eval η (ELet [ Binding p e' ] e) @ E <|Ψ|> {{ φ' }}.
  Proof.
    iIntros "He' HP He".
    iApply (ewp_ELet (λ δ, EWP[ι]eval (δ ++ η) e @ E <|Ψ|> {{ φ' }})%I
      with "[He' HP He]").
  ...
  Lemma ewp_ELet_PVar_1 {η x e e'} (Φ : val -> iProp Σ) φ E Ψ:
    EWP[ι]eval η e' @ E <|Ψ|> {{ ensures v, Φ v }} -∗
    (∀ v, Φ v -∗ EWP[ι]eval ((x, v) :: η) e @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι]eval η (ELet [Binding (PVar x) e'] e) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H P".
  ...
  Lemma ewp_ELet_PVar_1_forward {η x e e' v} R φ E Ψ:
    EWP[ι]eval η e' @ E <|Ψ|> {{ RET= v, R }} -∗
    (R -∗ EWP[ι]eval ((x, v) :: η) e @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι]eval η (ELet [ Binding (PVar x) e' ] e) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H P".
  ...
  Lemma ewp_ELet_PAny_1 {η e e'} R φ E Ψ:
    EWP[ι]eval η e' @ E <|Ψ|> {{ ensures _, R }} -∗
    (R -∗ EWP[ι]eval η e @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι]eval η (ELet [Binding PAny e'] e) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H P".
    iApply (ewp_ELet_singleton_total with "H").
    iIntros (v) "_".
  ...
  Lemma ewp_ELetRec η bs e φ E Ψ :
    ...
    EWP[ι]eval (δ ++ η) e @ E <|Ψ|> {{ φ }} -∗
    EWP[ι]eval η (ELetRec bs e) @ E <|Ψ|> {{ φ }}.
  Proof.
    simpl_eval; auto.
  Qed.
  ...
  Lemma ewp_ELetRec_specced_1 {η f x e1 e2 φ E Ψ} {A : Type} (R : A → val → iProp Σ) φf :
    □(∀ vf,
     □(∀ a v, R a v -∗ EWP[ι]call vf v <|Ψ|> {{ φf a }}) -∗
       ∀ a v, R a v -∗ EWP[ι]eval ((x, v) :: (f, vf) :: η) e1 <|Ψ|> {{ φf a }}) -∗
    (∀ vf,
      □(∀ a v, R a v -∗ EWP[ι]call vf v <|Ψ|> {{ φf a }}) -∗
      EWP[ι]eval ((f, vf) :: η) e2 @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι]eval η (ELetRec [RecBinding f (AnonFun x e1)] e2) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "#He1 He2".
  ...
  Lemma ewp_ELetRec_specced_ret_1 {η f x e1 e2 φ E Ψ} {A} (R : A → val → iProp Σ) φf :
    □(∀ vf,
     □(∀ a v, R a v -∗ EWP[ι]call vf v <|Ψ|> {{ ensures v', φf a v' }}) -∗
       ∀ a v, R a v -∗ EWP[ι]eval ((x, v) :: (f, vf) :: η) e1 <|Ψ|> {{ ensures v', φf a v' }}) -∗
    (∀ vf,
      □(∀ a v, R a v -∗ EWP[ι]call vf v <|Ψ|> {{ ensures v', φf a v' }}) -∗
      EWP[ι]eval ((f, vf) :: η) e2 @ E <|Ψ|> {{ φ }}) -∗
    EWP[ι]eval η (ELetRec [RecBinding f (AnonFun x e1)] e2) @ E <|Ψ|> {{ φ }}.
  ...
  Lemma ewp_ELetOpen η me e E ψ φ :
    EWP[ι]eval_mexpr η me @ E <|ψ|> {{ ensures m, ∃ δ, ⌜ m = VStruct δ ⌝ ∗
    EWP[ι]eval (δ ++ η) e @ E <|ψ|> {{ φ }} }} -∗
    EWP[ι]eval η (ELetOpen me e) @ E <|ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_ESeq_exn η e1 e2 φ1 φ E Ψ :
    EWP[ι]eval η e1 @ E <|Ψ|> {{ φ1 }} -∗
    (∀ v, φ1 (O2Throw v) -∗ φ (O2Throw v)) -∗
    (∀ v1, φ1 (O2Ret v1) -∗ EWP[ι]eval η e2 @ E <|Ψ|> {{ φ }}) -∗
  ...
  Lemma ewp_EIfThen_exn' η eb e1 φb φ E Ψ :
    EWP[ι] eval η eb @ E <|Ψ|> {{ φb }} -∗
    (∀ v, φb (O2Throw v) -∗ φ (O2Throw v)) -∗
    (∀ v, φb (O2Ret v) -∗
      (⌜v = VTrue ⌝ ∗ EWP[ι] eval η e1 @ E <|Ψ|> {{ φ }})
    ∨ (⌜v = VFalse⌝ ∗ φ (O2Ret VUnit))) -∗
    EWP[ι] eval η (EIfThen eb e1) @ E <|Ψ|> {{ φ }}.
  Proof.
  ...
  Lemma ewp_EIfThen_exn η eb e1 φb φ E Ψ :
    ...
    (∀ v, φb (O2Ret v) -∗
      ∃ b, ⌜v = VBool b⌝ ∗
      if b then EWP[ι]eval η e1 @ E <|Ψ|> {{ φ }}
           else φ (O2Ret VUnit)) -∗
    EWP[ι]eval η (EIfThen eb e1) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "Hb E P /=". simpl_eval.
  ...
  Lemma ewp_EIfThenElse_exn' η eb e1 e2 φb φ E Ψ :
    ...
    (∀ v, φb (O2Ret v) -∗
      (⌜v = VTrue ⌝ ∗ EWP[ι] eval η e1 @ E <|Ψ|> {{ φ }})
    ∨ (⌜v = VFalse⌝ ∗ EWP[ι] eval η e2 @ E <|Ψ|> {{ φ }})) -∗
    EWP[ι] eval η (EIfThenElse eb e1 e2) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "Hb E P /=". simpl_eval.
    iApply ewp_bind'.
    iApply ewp_bind'.
  ...
  Lemma ewp_EMatch η e bs Ψ E Φ :
    ...
    EWP[ι]eval η (EMatch e bs) @ E <|Ψ|> {{ Φ }}.
  Proof.
  ...
  Lemma ewp_ERaise η e φ E Ψ :
    EWP[ι] eval η e @ E <|Ψ|> {{ RET v => φ (O2Throw v) | EXN v => φ (O2Throw v)}} -∗
    EWP[ι] eval η (ERaise e) @ E <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H". simpl_eval.
    iApply ewp_bind'.
    iApply (ewp_mono with "H").
  ...
  Lemma ewp_EAssert η e R E Ψ :
    R ∧ EWP[ι]eval η e @ E <|Ψ|> {{ ensures v, ⌜v = VTrue⌝ ∗ R }} -∗
    EWP[ι]eval η (EAssert e) @ E <|Ψ|> {{ ensures _, R }}.
  Proof.
    iIntros "H".
  ...
  Lemma ewp_EAssert_sep η e R E Ψ :
    R ∗ (R -∗ EWP[ι]eval η e @ E <|Ψ|> {{ ensures v, ⌜v = VTrue⌝ ∗ R }}) -∗
    EWP[ι]eval η (EAssert e) @ E <|Ψ|> {{ ensures _, R }}.
  Proof.
    iIntros "(? & ?)".
  ...
  Lemma ewp_ERef `{Encode A} η e (φ1 : A → iProp Σ) (φ : loc → iProp Σ) E Ψ :
    EWP[ι] eval η e @ E <|Ψ|> {{ ensures #a, φ1 a }} -∗
    (∀ (a : A) (l : loc), φ1 a -∗ l ↦ V #a -∗ φ l) -∗
    EWP[ι] eval η (ERef e) @ E <|Ψ|> {{ ensures #l, φ l }}.
  Proof.
    iIntros "H P".
  ...
  Lemma ewp_ELoad η e (φ1 : loc → iProp Σ) φ E Ψ :
    EWP[ι] (eval η e) @ E <|Ψ|> {{ ensures #l, φ1 l }} -∗
    (∀ l, φ1 l -∗ ∃ q v, pointsto l q (V v) ∗ ▷(pointsto l q (V v) -∗ φ v)) -∗
    EWP[ι] eval η (ELoad e) @ E <|Ψ|> {{ ensures v, φ v }}.
  Proof.
    iIntros "H P".
    iApply (ewp_ELoad_exn with "[H]"); auto.
    - iApply ewp_bind. iApply (ewp_mono with "H").
      iIntros ([|]); [ iIntros "Hl" | iIntros ([])].
      simpl. iDestruct "Hl" as "(%v & -> & Hφ1)".
      iApply ewp_ret. instantiate (1 := (ensures v, φ1 v)%I). iApply "Hφ1".
    - iIntros (?) "[]".
    - iIntros (l) "Hφ1".
      iApply "P". iApply "Hφ1".
  Qed.
  ...
  Lemma ewp_ELoad_simple η e (l : loc) q (v : val) E Ψ :
    EWP[ι] eval η e @ E <|Ψ|> {{ RET= #l }} -∗
    pointsto l q (V v) -∗ EWP[ι] eval η (ELoad e) @ E <|Ψ|> {{ RET= v, pointsto l q (V v) }}.
  Proof.
  ...
  Lemma ewp_EStore `{Encode A} {η e1 e2} (φ1 : loc -d> iPropO Σ) (φ2 : A -d> iPropO Σ) φ E Ψ :
    EWP[ι] eval η e1 @ E <|Ψ|> {{ ensures #l, φ1 l }} -∗
    EWP[ι] eval η e2 @ E <|Ψ|> {{ ensures #a, φ2 a }} -∗
    (∀ l a, φ1 l ∗ φ2 a -∗
            ∃ (a' : A), l ↦ (V #a') ∗ ▷ (l ↦ (V #a) -∗ φ VUnit)) -∗
    EWP[ι] eval η (EStore e1 e2) @ E <|Ψ|> {{ ensures v, φ v }}.
  Proof.
    iIntros "H1 H2 P".
    iApply (ewp_EStore_exn with "[H1] H2").
    - iApply ewp_bind; iApply (ewp_mono with "H1").
  ...
  (* Here, [l↦v] is threaded through [e2], as one is more likely to use
     ...
     [(!r := 1; r) := 2]) *)
  Lemma ewp_EStore_simple `{Encode A} η e1 e2 (l : loc) (v : A) E Ψ :
    EWP[ι] eval η e1 @ E <|Ψ|> {{ RET= #l }} -∗
    EWP[ι] eval η e2 @ E <|Ψ|> {{ RET= #v, l ↦ (V #v) }} -∗
    EWP[ι] eval η (EStore e1 e2) @ E <|Ψ|> {{ RET= #(), l ↦ (V #v) }}.
  Proof.
    iIntros "H1 H2".
    iApply (ewp_EStore (λ l1, ⌜l = l1⌝%I) (λ v', ⌜v'=v⌝ ∗ l ↦ V #v)%I  with "[H1] [H2]").
    - iApply (ewp_mono with "H1").
      iIntros ([|]); [ | iIntros "[]" ].
      iIntros "(-> & _)".
      iExists l; auto.
    - iApply (ewp_mono with "H2").
      iIntros ([|]); [ | iIntros "[]" ].
      iIntros "(-> & Hl)".
      iExists v; auto.
    - iIntros (? ?) "(-> & -> & Hl)". iFrame.
  ...
  Lemma ewp_EPerform η e E ψ (φ1 : val -d> iPropO Σ) φ :
    EWP[ι]eval η e @ E <|ψ|> {{ ensures v, φ1 v }} -∗
    (∀ v, φ1 v -∗ EWP[ι]perform v @ E <|ψ|> {{ φ }} ) -∗
    EWP[ι]eval η (EPerform e) @ E <|ψ|> {{ φ }}.
  Proof.
    iIntros "He Hv".
