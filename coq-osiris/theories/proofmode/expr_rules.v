...
Section ewp_rules_expr.
  ...
  Lemma ewp_eval_bindings_cons_total η p e bs φ1 φs φ Ψ :
  ...
  Lemma ewp_list_nil η Ψ :
    ⊢ ([∗ list] ei;φi ∈ []; [], EWP eval η ei <|Ψ|> {{ φi }}).
  Proof. done. Qed.

  Lemma ewp_list_cons η e es φ φs Ψ :
    ([∗ list] ei;φi ∈ (e :: es);(φ :: φs), EWP eval η ei <|Ψ|> {{ φi }}) ⊣⊢
    EWP eval η e <|Ψ|> {{ φ }} ∗ [∗ list] ei;φi ∈ es;φs, EWP eval η ei <|Ψ|> {{ φi }}.
  Proof. done. Qed.

  Lemma ewp_list_singleton η e φ Ψ :
    ([∗ list] ei;φi ∈ [e]; [φ], EWP eval η ei <|Ψ|> {{ φi }}) ⊣⊢
    EWP eval η e <|Ψ|> {{ φ }}.
  ...
  Lemma ewp_EPair_ret η e1 e2 φ1 φ2 Ψ φ :
    EWP eval η e1 <|Ψ|> {{ RET v, φ1 v }} -∗
    EWP eval η e2 <|Ψ|> {{ RET v, φ2 v }} -∗
    (∀ v1 v2, φ1 v1 -∗ φ2 v2 -∗ φ #(v1, v2)) -∗
    EWP eval η (EPair e1 e2) <|Ψ|> {{ RET v, φ v }}.
  Proof.
    iIntros "H1 H2 P".
    iApply (ewp_EPair_exn with "H1 H2 [P]"); auto.
  Qed.

  Lemma ewp_EPair_ret_encode η e1 e2 `{Encode A} `{Encode B} (v1 : A) (v2 : B) R1 R2 Ψ :
  ...
  Corollary ewp_ELet_singleton_total {η p e e'} Φ φ Ψ:
    EWP eval η e' <|Ψ|> {{ RET v, Φ v }} -∗
    (∀ v, Φ v -∗
        ∃ δ,
          ⌜simp (irrefutably_extend nil p v) (ret δ)⌝ ∗
          EWP eval (δ ++ η) e <|Ψ|> {{ φ }}) -∗
    EWP eval η (ELet [ Binding p e' ] e) <|Ψ|> {{ φ }}.
  Proof.
    iIntros "He HP".
    iApply (ewp_ELet (λ δ, EWP eval (δ ++ η) e <|Ψ|> {{ φ }})%I
      with "[He HP]").
  ...
  Lemma ewp_ELet_PVar_1 {η x e e'} Φ φ Ψ:
    EWP eval η e' <|Ψ|> {{ RET v, Φ v }} -∗
    (∀ v, Φ v -∗ EWP eval ((x, v) :: η) e <|Ψ|> {{ φ }}) -∗
    EWP eval η (ELet [Binding (PVar x) e'] e) <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H P".
  ...
  (* Alternative form *)
  Lemma ewp_ELet_PVar_1_forward {η x e e' v} R φ Ψ:
    EWP eval η e' <|Ψ|> {{ RET= v, R }} -∗
    (R -∗ EWP eval ((x, v) :: η) e <|Ψ|> {{ φ }}) -∗
    EWP eval η (ELet [ Binding (PVar x) e' ] e) <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H P".
  ...
  Lemma ewp_ELet_PAny_1 {η e e'} R φ Ψ:
    EWP eval η e' <|Ψ|> {{ RET _, R }} -∗
    (R -∗ EWP eval η e <|Ψ|> {{ φ }}) -∗
    EWP eval η (ELet [Binding PAny e'] e) <|Ψ|> {{ φ }}.
  Proof.
    iIntros "H P".
    iApply (ewp_ELet_singleton_total with "H").
    iIntros (v) "Hv".
