...
Lemma pure_eval_pair `{Encode A1, Encode A2} η e1 e2 (ψ : A1 * A2 → Prop) :
  pure_enc (eval η e1) (λ a1 : A1, pure_enc (eval η e2) (λ a2 : A2, ψ (a1, a2))) →
  pure_enc (eval η (EPair e1 e2)) ψ.
Proof.
  intros. simpl_eval.
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
  repeat apply pure_ret.
  eauto with encode.
Qed.

Lemma pure_eval_triple `{Encode A1, Encode A2, Encode A3} η e1 e2 e3 (ψ : A1 * A2 * A3 -> Prop) :
  pure_enc (eval η e1) (λ a1 : A1,
        pure_enc (eval η e2) (λ a2 : A2,
              pure_enc (eval η e3) (λ a3 : A3,
                    ψ (a1, a2, a3)))) ->
  pure_enc (eval η (ETuple [e1; e2; e3])) ψ.
Proof.
  intros. simpl_eval.
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
...
Lemma pure_eval_quadruple `{Encode A1, Encode A2, Encode A3, Encode A4} (η : env) (e1 e2 e3 e4 : expr)
  ...
  pure_enc (eval η e1) (λ a1 : A1,
        pure_enc (eval η e2) (λ a2 : A2,
              pure_enc (eval η e3) (λ a3 : A3,
                    pure_enc (eval η e4) (λ a4 : A4,
                          ψ (a1, a2, a3, a4))))) ->
  pure_enc (eval η (ETuple [e1; e2; e3; e4])) ψ.
Proof.
  intros. simpl_eval.
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
...
Lemma pure_eval_pair_val η e1 e2 (ψ : val → Prop) :
  pure_enc (eval η e1) (λ a1 : val, pure_enc (eval η e2) (λ a2 : val, ψ (VPair a1 a2))) →
  pure_enc (eval η (EPair e1 e2)) ψ.
Proof.
  intros. simpl_eval.
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
...
:
  pure_enc (eval η e1) φ1 →
  pure_enc (eval η e2) φ2 →
...
Proof.
  ...
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
  eapply purev_Par_left_conseq; eauto. intros ? (? & -> & ?).
  repeat apply pure_ret. eauto with encode.
Qed.
...
Lemma pure_eval_anonfun `{Encode A, Encode A1} η v e (φ : A1 -> Prop) (ψ : val -> Prop) :
  (∀ (x : A), pure_enc (eval ((v, #x) :: η) e) φ) ->
  (∀ (vf : val), (∀ (x : A), pure_enc (call vf #x) φ) -> ψ vf) ->
  pure_enc (eval η (EAnonFun (AnonFun v e))) ψ.
Proof.
...
Lemma pure_eval_anonfunction `{Encode A, Encode A1} η bs (φ : A1 -> Prop) (ψ : val -> Prop) :
  (∀ (x : A),
      pure_enc
        ...
        φ) ->
  (∀ (vf : val), (∀ (x : A), pure_enc (call vf #x) φ) -> ψ vf) ->
  pure_enc (eval η (EAnonFun (AnonFunction bs))) ψ.
Proof.
  intros Hcall Hcov.
  eapply pure_simp; [ simp | eauto ].
  eapply pure_enc_ret; first solve [encode].
...
Lemma pure_eval_app `{Encode A1, Encode A} η e1 e2 (ψ : A → Prop) :
  pure_enc (eval η e1)
    (λ f : val,
        pure_enc (eval η e2)
          (λ arg : A1,
              pure_enc (call f #arg) ψ)) →
  pure_enc (eval η (EApp e1 e2)) ψ.
Proof.
...
:
  pure_enc (eval η e1) φ1 →
  pure_enc (eval η e2) φ2 →
  (∀ v1 v2, φ1 v1 → φ2 v2 → pure_enc (call v1 #v2) ψ) →
  pure_enc (eval η (EApp e1 e2)) ψ.
Proof.
...
Definition pure_call2 `{Encode X} vf arg1 arg2 (φ : X -> Prop) :=
  pure_enc (call vf arg1)
    (λ c, pure_enc (call c arg2) φ).

Lemma pure_eval_app2 `{Encode A, Encode B, Encode C} η e1 e2 e3 vf
  (arg1 : A) (arg2 : B) (ψ : C → Prop)
  :
  pure_enc (eval η e1) (λ vf', vf' = vf) ->
  pure_enc (eval η e2) (λ arg1', arg1' = arg1) ->
  pure_enc (eval η e3) (λ arg2', arg2' = arg2) ->
  pure_call2 vf #arg1 #arg2 ψ ->
  pure_enc (eval η (EApp (EApp e1 e2) e3)) ψ.
Proof.
...
Lemma pure_as_int m (φ : Z → Prop) :
  pure_enc m φ →
  pure (as_int m) (λ i, ∃ z, i = repr z ∧ φ z) (λ _, False).
Proof.
...
Lemma pure_enc_par_as_int m1 m2 (φ1 φ2 φ : Z → Prop) k :
  pure_enc m1 φ1 →
  pure_enc m2 φ2 →
  (∀ z1 z2 : Z, φ1 z1 → φ2 z2 → pure_enc (k (repr z1, repr z2)) φ) →
  pure_enc (Par (as_int m1) (as_int m2) (pfbind inject2 k)) φ.
Proof.
  intros Hm1 Hm2 Hk.
  unfold as_int.
  eapply pure_Par_conseq. 1,2: eapply pure_as_int; eauto. 2: tauto.
...
Lemma pure_eval_add η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η (EIntAdd e1 e2)) φ.
Proof.
...
Lemma pure_eval_sub η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η e2) φ2 →
  (∀ z1 z2, φ1 z1 → φ2 z2 → φ (z1 - z2)) →
  pure_enc (eval η (EIntSub e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
  eapply pure_enc_ret; eauto with encode.
Qed.
...
Lemma pure_eval_mul η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η e2) φ2 →
  (∀ z1 z2, φ1 z1 → φ2 z2 → φ (z1 * z2)) →
  pure_enc (eval η (EIntMul e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
  eapply pure_enc_ret; eauto with encode.
Qed.
...
Lemma pure_eval_div η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  (∀ z2, φ2 z2 → z2 ≠ 0) →
  (∀ z1 z2, φ1 z1 → φ2 z2 → φ (z1 ÷ z2)) →
  pure_enc (eval η (EIntDiv e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
...
Lemma pure_eval_land η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η (EIntLand e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
  eapply pure_enc_ret; eauto with encode.
Qed.
...
Lemma pure_eval_lor η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η (EIntLor e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
  eapply pure_enc_ret; eauto with encode.
Qed.
...
Lemma pure_eval_lxor η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η (EIntLxor e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
  eapply pure_enc_ret; eauto with encode.
Qed.
...
Lemma pure_eval_lsl η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η (EIntLsl e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
...
Lemma pure_eval_lsr η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  pure_enc (eval η (EIntLsr e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
  apply pure_if_in_shift_range; auto.
  eapply pure_enc_ret; eauto with encode.
...
Lemma pure_eval_asr η e1 e2 (φ1 φ2 φ : Z → Prop) :
  ...
  (∀ z2, φ2 z2 → in_shift_range z2) →
  (∀ z1 z2, φ1 z1 → φ2 z2 → φ (Z.shiftr z1 z2)) →
  pure_enc (eval η (EIntAsr e1 e2)) φ.
Proof.
  intros. simpl_eval. eapply pure_enc_par_as_int; eauto. intros.
  apply pure_if_in_shift_range; auto.
  eapply pure_enc_ret; eauto with encode.
...
Lemma pure_enc_bind_as_bool {A} m (φ : bool → Prop) (ψ : A → Prop) (k : bool → micro A exn) :
  pure_enc m φ →
  (∀ b : bool, φ b → pure (k b) ψ (λ _, False)) →
  pure (bind (as_bool m) k) ψ (λ _, False).
Proof.
...
Lemma pure_eval_let_pair `{Encode A1, Encode A2} `{Encode X}
  ...
  pure_enc (eval η e1) (λ '((v1, v2) : A1 * A2),
      pure_enc (
          δ ← widen (irrefutably_extend [] p1 #v1);
          θ ← widen (irrefutably_extend δ p2 #v2);
          eval (θ ++ η) e2
        ) ψ) ->
  pure_enc (eval η (ELet1 (PPair p1 p2) e1 e2)) ψ.
Proof.
...
Lemma pure_eval_let `{Encode A1, Encode B} η x e1 e
  (φ1 : A1 → Prop) (ψ : B → Prop) :
  pure_enc (eval η e1) φ1 →
  (∀ a1, φ1 a1 → pure_enc (eval ((x, #a1) :: η) e) ψ) →
  pure_enc (eval η (ELet1Var x e1 e)) ψ.
Proof.
  intros He1 He.
  simpl_eval.
...
Lemma pure_eval_let' `{Encode A1, Encode B} η x e1 e
  (a1 : A1) (ψ : B → Prop) :
  pure_enc (eval η e1) (λ x, x = a1) →
  pure_enc (eval ((x, #a1) :: η) e) ψ →
  pure_enc (eval η (ELet1Var x e1 e)) ψ.
Proof.
  intros He1 He2.
...
Lemma pure_eval_ifthenelse_bool `{Encode A} η e e1 e2 (φ : bool → Prop) (ψ : A → Prop) :
  pure_enc (eval η e) φ →
  (φ true  → pure_enc (eval η e1) ψ) →
  (φ false → pure_enc (eval η e2) ψ) →
  pure_enc (eval η (EIfThenElse e e1 e2)) ψ.
Proof.
  intros. eapply pure_ifthenelse_bool; eauto.
Qed.

Lemma pure_eval_ifthenelse `{Encode A} η e e1 e2 (P : Prop) (ψ : A → Prop) :
  pure_enc (eval η e) (λ P', P' <-> P) →
  (P → pure_enc (eval η e1) ψ) →
  (~ P → pure_enc (eval η e2) ψ) →
  pure_enc (eval η (EIfThenElse e e1 e2)) ψ.
Proof.
  intros He He1 He2.
...
Lemma pure_eval_comparison_operator `{Encode A} η e1 e2 (x1 x2 : Z) (f : val → val → micro bool exn) b a (φ : A → Prop) :
  pure_enc (eval η e1) (λ x1' : Z, x1' = x1) →
  pure_enc (eval η e2) (λ x2' : Z, x2' = x2) →
  f #x1 #x2 = ret b →
  #a = #b →
  φ a →
  pure_enc (Par (eval η e1) (eval η e2) (pfbind inject2 (λ '(v1, v2), 'b ← f v1 v2; ret (VBool b)))) φ.
Proof.
...
Lemma pure_eval_EOpLe η e1 e2 (x1 x2 : Z) :
  pure_enc (eval η e1) (λ x1', x1' = x1) ->
  pure_enc (eval η e2) (λ x2', x2' = x2) ->
  (* Representability hypotheses last for [x1] and [x2] evar initialisation *)
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpLe e1 e2)) (λ P, P <-> (x1 <= x2)%Z).
Proof.
...
Lemma pure_eval_EOpLe_bool η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpLe e1 e2)) (λ (b : bool), b <-> (x1 <= x2)).
Proof.
...
Lemma pure_eval_EOpLt η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpLt e1 e2)) (λ P, P <-> (x1 < x2)%Z).
Proof.
  intros He1 He2 Hx1 Hx2. simpl_eval.
  eapply pure_eval_comparison_operator; eauto.
  rewrite lt_repr_repr; auto.
...
Lemma pure_eval_EOpLt_bool η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpLt e1 e2)) (λ (b : bool), b <-> (x1 < x2)).
Proof.
...
Lemma pure_eval_EOpGt η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpGt e1 e2)) (λ P, P <-> (x1 > x2)%Z).
Proof.
  intros He1 He2 Hx1 Hx2. simpl_eval.
  eapply pure_eval_comparison_operator; eauto.
  rewrite lt_repr_repr; auto.
...
Lemma pure_eval_EOpGt_bool η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpGt e1 e2)) (λ (b : bool), b <-> (x1 > x2)).
Proof.
...
Lemma pure_eval_EOpGe η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpGe e1 e2)) (λ P, P <-> (x1 >= x2)%Z).
Proof.
  intros He1 He2 Hx1 Hx2. simpl_eval.
  eapply pure_eval_comparison_operator; eauto.
  rewrite lt_repr_repr; auto.
...
Lemma pure_eval_EOpGe_bool η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpGe e1 e2)) (λ (b : bool), b <-> (x1 >= x2)).
Proof.
...
Lemma pure_eval_EOpEq η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpEq e1 e2)) (λ P, P <-> (x1 = x2)%Z).
Proof.
  intros He1 He2 Hx1 Hx2. simpl_eval.
  eapply pure_eval_comparison_operator; eauto.
...
Lemma pure_eval_EOpEq_bool η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpEq e1 e2)) (λ (b : bool), b <-> (x1 = x2)).
Proof.
...
Lemma pure_eval_EOpNe η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpNe e1 e2)) (λ P, P <-> (x1 <> x2)%Z).
Proof.
  intros He1 He2 Hx1 Hx2. simpl_eval.
  eapply pure_eval_comparison_operator; eauto.
  rewrite eq_repr_repr; auto.
...
Lemma pure_eval_EOpNe_bool η e1 e2 (x1 x2 : Z) :
  ...
  representable x1 ->
  representable x2 ->
  pure_enc (eval η (EOpNe e1 e2)) (λ (b : bool), b <-> (x1 <> x2)).
Proof.
...
Lemma pure_eval_assert_bool η e :
  pure_enc (eval η e) (λ b, b = true) →
  pure_enc (eval η (EAssert e)) (λ (_ : unit), True).
Proof.
  intros He.
...
Lemma pure_eval_seq' `{Encode A} η e1 e2 (ψ : A -> Prop) :
  pure (eval η e1) (λ _, True) (λ _, False) ->
  pure_enc (eval η e2) ψ ->
  pure_enc (eval η (ESeq e1 e2)) ψ.
Proof.
  intros He1 He2.
  apply pure_seq.
...
Lemma pure_eval_mexpr_struct (η δ whatenv : env) items (ψ : val -> Prop) :
  ...
  ψ (VStruct δ) ->
  pure_enc (eval_mexpr η (MStruct items)) ψ.
Proof.
...
Lemma pure_eval_mexpr_coerc η me c v (ψ : val -> Prop):
  ...
  pure_enc (coerce c v) ψ ->
  pure_enc (eval_mexpr η (MCoercion me c)) ψ.
Proof.
...
Lemma pure_eval_path `{Encode A} η π (ψ : A -> Prop) :
  pure_enc (lookup_path η π) ψ ->
  pure_enc (eval η (EPath π)) ψ.
Proof.
  simpl_eval.
  apply pure_widen.
Qed.
...
Lemma pure_eval_data `{Encode Y} η c e y (ψ : Y -> Prop) :
  pure_enc (eval η e) (λ v', VData c v' = #y) ->
  ψ y ->
  pure_enc (eval η (EData c e)) ψ.
Proof.
  intros He Hy.
  simpl_eval.
...
Lemma pure_eval_data1 `{CRel1 A1 X c C} (η : env) (e : expr) (ψ : X → Prop) :
  ...
  pure_enc (eval η (EData c (ETuple [e]))) ψ.
Proof.
  intros He.
  simpl_eval.
  apply pure_Par_vals_left.
