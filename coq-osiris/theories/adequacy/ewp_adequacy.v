...
Section satisfiability_weakest_pre.
  ...
  Proof.
    ...
    - (* Case: [e] takes a thread_step. *)
      ...
      + (* Subcase: [e] is a join. *)
        discriminate Hhm.
      + (* Subcase: [e] is a fork. *)
        epose proof (ForkS σ ι (dom πp) _ _ k (fresh (dom πp)) (is_fresh _)) as Htstep_fork.
        spec_step.
      ...
      + (* Subcase: [e] is a self. *)
        epose proof (SelfS σ (dom πp) ι u k) as Htstep_self.
        spec_step.
        iModIntro. iNext. iMod "Hwp". iModIntro.
        iSplitR.
        * (* Prove that we are not stuck. *)
          iPureIntro. right; left. apply can_progress_self.
        * (* Prove that if we are final, we satisfy the post (we are not final). *)
          iIntros (o HFalse). destruct o; discriminate HFalse.
      + (* Subcase: [e] takes a regular step. *)
        pose proof (BaseS e σ ι _ _ (dom πp) Hstep) as Htstep_base.
  ...
  Proof.
    ...
    - (* BaseTP case: π !! ι = Some m, step (σ, m) (σ', m') *)
      ...
      exists ι, m, m', None.
      split; [exact Hlookup | split; [apply lookup_insert | split; [exact Htstep | reflexivity]]].
    - (* ForkTP case: π !! ι = Some (Stop CFork ...), π !! ι' = None *)
      ...
      split; [exact Hlookup | ].
      split; [rewrite lookup_insert_ne; [apply lookup_insert | ] | ].
    ...
    - (* SelfTP case: π !! ι = Some (Stop CSelf ...) *)
      right; rename H into Hlookup; subst.
      exists ι, (Stop CSelf () k), (continue k (VThread ι)), None.
      split; [exact Hlookup | split; [apply lookup_insert | split; [apply SelfS | reflexivity]]].
  Qed.
  ...
  Proof.
    ...
    - (* Case: [threadpool_step] is immitated by a [thread_step]. *)
      ...
      + (* Subcase: the step results in a forked thread. *)
        ...
        rewrite fmap_insert.
        iFrame.
        iPureIntro. intros ι'' Hin. apply elem_of_list_singleton in Hin as ->.
        rewrite Hdomeq. apply not_elem_of_dom.
        assert (ι ≠ ι') as Hneq_ι.
        { intros Heq_ι. subst ι'. rewrite lookup_insert in Hfresh. discriminate Hfresh. }
