...
Lemma invert_step_install {A E} σ σ' l η bs k sk m' :
  σ !! l = Some (K sk) →
  @step A E (σ, Stop CInstall (true, l, η, bs) k) (σ', m') →
  ∃ l',
  σ !! l' = None ∧
  σ' = <[ l' := K (λ o, Handle (sk o) (λ o, eval_match true η o bs)) ]> σ ∧
  m' = continue k l'.
Proof.
  intros Heq Hstep. destruct_step.
...
Lemma invert_step_shallow_install {A E} σ σ' l η bs k sk m' :
  σ !! l = Some (K sk) →
  @step A E (σ, Stop CInstall (false, l, η, bs) k) (σ', m') →
  ∃ l',
  σ !! l' = None ∧
  σ' = <[ l' := K (λ o, Handle (sk o) (λ o, eval_match false η o bs)) ]> σ ∧
  m' = continue k l'.
Proof.
  intros Heq Hstep. destruct_step.
  unfold step_install_1, step_install_2. rewrite Heq.
