...
(* A naive rule for recursive calls on a single argument.
   It is used later in the proof of [pure_letrec]. *)

Local Lemma pure_rec_call `{Encode X, Encode Y}
...
Proof.
  ...
  simpl; rewrite String.eqb_refl; apply pure_CEval_inject2.
  apply Hrec; [ intros y HR HPy | assumption ].
  apply IH; auto.
Qed.

(* A reasoning rule when facing an expression of the form [let rec f x = e1 in e2]. *)
...
Local Lemma pure_letrec `{Encode X, Encode Y} {A}
  ...
  (∀ vf (x : X) a,
      (∀ (y : X), R y x -> P a y -> pure (call vf #y) (φf a y) Ψ) ->
...
(* The same reasoning rule as before, but without the auxiliary variable [a]. *)

Local Lemma pure_letrec_simple `{Encode X, Encode Y}
...
(* An even simpler reasoning rule, without a precondition on the argument. *)

Local Lemma pure_rec_call_no_pre `{Encode X} `{Encode Y}
...
(* What follows is the same scheme but for functions with two
   arguments. In particular, we have a second argument [y : Y], and the
   function to return a value of type [W]. The decreasing relation [R]
   is now of type [(X * Y) -> (X * Y) -> Prop]. *)
...
Lemma pure_rec_call2 `{Encode X, Encode Y, Encode W}
  ...
  (∀ vf x1 y1,
      (∀ (x2 : X) (y2 : Y),
          R (x2, y2) (x1, y1) ->
          P x2 y2 ->
          pure (call vf #x2) (λ c, pure (call c #y2) (φ x2 y2) Ψ) Ψ) ->
      (P x1 y1 ->
       pure (eval ((arg, #x1) :: (f, vf) :: η) e1)
         (λ c, pure (call c #y1) (φ x1 y1) Ψ)
         Ψ)) ->
  pure (call (VCloRec η [RecBinding f (AnonFun arg e1)] f) #x)
    (λ c, pure (call c #y) (φ x y) Ψ)
    Ψ.
Proof.
  intros Hwf HP Hrec.
...
Lemma pure_letrec2 `{Encode X, Encode Y, Encode W} (A : Type)
  η f arg1 arg2 e1 e2 (φ : val -> Prop) Ψ
  (φf : A -> X -> Y -> W -> Prop)
  (R : (X * Y) -> (X * Y) -> Prop) (P : A -> X -> Y -> Prop) (ζ : A -> exn -> Prop) :
  wf R ->
  (∀ vf x1 y1 a,
      (∀ x2 y2, R (x2, y2) (x1, y1) ->
                P a x2 y2 ->
                pure (call vf #x2)
                  (λ c, pure (call c #y2) (φf a x2 y2) (ζ a))
                  (ζ a)) ->
      P a x1 y1 ->
      pure (eval ((arg2, #y1) :: (arg1, #x1) :: (f, vf) :: η) e1) (φf a x1 y1) (ζ a)) ->
  (∀ vf,
      (∀ a x y, P a x y ->
                pure (call vf #x)
                  (λ c, pure (call c #y) (φf a x y) (ζ a))
                  (ζ a)) ->
      pure (eval ((f, vf) :: η) e2) φ Ψ) ->
  pure (eval η (ELetRec [RecBinding f (AnonFun arg1 (EAnonFun (AnonFun arg2 e1)))] e2)) φ Ψ.
Proof.
  intros Hwf He1 He2.
  simpl_eval. apply He2; clear He2.
  intros a x y HP.
  eapply pure_rec_call2; eauto.
  intros vf x1 y1 IH HP2.
  simpl_eval; eapply pure_ret; encode.
  apply pure_CEval_inject2.
  eapply He1; eauto.
Qed.

Lemma pure_letrec2_simple `{Encode X, Encode Y, Encode W, Encode T}
  η f arg1 arg2 e1 e2 (φ : T -> Prop) Ψ
  (φf : X -> Y -> W -> Prop)  (R : (X * Y) -> (X * Y) -> Prop) (P : X -> Y -> Prop) (ζ : exn -> Prop) :
  wf R ->
  (∀ vf x1 y1,
      (∀ x2 y2, R (x2, y2) (x1, y1) ->
                P x2 y2 ->
                pure (call vf #x2)
                  (λ c, pure (call c #y2) (φf x2 y2) ζ)
                  ζ )->
      P x1 y1 ->
      pure (eval ((arg2, #y1) :: (arg1, #x1) :: (f, vf) :: η) e1) (φf x1 y1) ζ) ->
  (∀ vf,
      (∀ x y, P x y ->
              pure (call vf #x)
                (λ c, pure (call c #y) (φf x y) ζ)
                ζ) ->
      pure (eval ((f, vf) :: η) e2) φ Ψ) ->
  pure (eval η (ELetRec [RecBinding f (AnonFun arg1 (EAnonFun (AnonFun arg2 e1)))] e2)) φ Ψ.
Proof.
...
Lemma pure_letrec2_no_pre `{Encode X, Encode Y, Encode W, Encode T}
  η f arg1 arg2 e1 e2 (φ : T -> Prop) Ψ
  (φf : X -> Y -> W -> Prop)  (R : (X * Y) -> (X * Y) -> Prop) (ζ : exn -> Prop) :
  wf R ->
  (∀ vf x1 y1,
      (∀ x2 y2, R (x2, y2) (x1, y1) ->
                pure (call vf #x2)
...
Proof.
  ...
  simpl_eval. apply He2; clear He2.
  intros x y.
  eapply pure_rec_call2 with (P := λ _ _, True); eauto.
  intros vf x1 y1 IH _.
  simpl_eval; eapply pure_ret; encode.
  apply pure_CEval_inject2.
  eapply He1; eauto.
Qed.
...
(* We have to duplicate our letrec lemmas for module-level letrecs. *)

Lemma struct_letrec_function `{Encode X, Encode Y} η δ f bs
  (φf : X -> Y -> Prop) (Ψ : envs -> Prop) (R : X -> X -> Prop) (P : X -> Prop) ζ
  :
  wf R ->
  (∀ vf x,
      (∀ y, R y x -> P y -> pure (call vf #y) (φf y) ζ) ->
      P x ->
...
Lemma struct_letrec2 `{Encode X, Encode Y, Encode W} (A : Type)
  η δ f arg1 arg2 e (Ψ : envs -> Prop)
  (φf : A -> X -> Y -> W -> Prop) (R : (X * Y) -> (X * Y) -> Prop)
  (P : A -> X -> Y -> Prop) (ζ : A -> exn -> Prop)
  :
  wf R ->
  (∀ vf x1 y1 a,
      (∀ x2 y2, R (x2, y2) (x1, y1) -> P a x2 y2 -> pure_call2 vf #x2 #y2 (φf a x2 y2) (ζ a)) ->
      P a x1 y1 ->
      pure (eval ((arg2, #y1) :: (arg1, #x1) :: (f, vf) :: η) e) (φf a x1 y1) (ζ a)) ->
  (∀ vf,
...
Proof.
  ...
  eapply pure_rec_call2; eauto.
  intros ?????.
  eapply pure_eval_anonfun. simpl. apply pure_CEval_inject2.
  eauto.
Qed.

Lemma struct_letrec `{Encode X, Encode Y} η δ f arg e1
  (φf : X -> Y -> Prop) (Ψ : envs -> Prop) (R : X -> X -> Prop) (P : X -> Prop) ζ
  :
  wf R ->
  (∀ vf x,
...
Lemma pure_eval_stop `{Encode A} η e (φ : A -> Prop) ζ :
  pure (stop CEval (η, e)) φ ζ -> pure (eval η e) φ ζ.
Proof.
  intros Hstop.
  inversion Hstop; subst.
  destruct H0 as [m' Hmay].
  specialize (H1 m' Hmay) as Hwp.
  pose proof (invert_may_eval _ _ _ Hmay) as ->; simpl in *.
  rewrite try2_ret_right in Hwp.
  apply Hwp.
Qed.

Lemma pure_acall_equiv `{Encode A} η a v (φ : A -> Prop) ζ :
  pure (eval.acall η a v) φ ζ <-> pure (fun_spec.acall η a v) φ ζ.
Proof.
  destruct a; simpl.
  split.
  { apply pure_eval_stop. }
  { apply pure_CEval_inject2. }
Qed.
...
Lemma pure_call_equiv `{Encode A} f v (φ : A -> Prop) ζ :
...
(* As an example, let us define [Spec'] as a way to specify unary functions. *)
...
(* Consider for example the specification of a function [sort]:
   [ Spec sort (λ l e, pure e (λ l', Sorted l' ∧ l' ≡ l) ⊥) ] *)

(* Example of a reasoning rule, for the creation of a [Spec c P]. *)

Local Lemma pure_eval_anon' `{Encode X} (P : call_spec') η (x : var) e ζ :
...
(* We want to be able to reason on n-ary functions. So we generalize
   this idea to take telescopes of arguments. *)
...
(* We can check on a simple example that [Spec] generalizes [Spec'].
   Notice that we have to give [Spec] an explicit telescope. *)
...
(* [weaken_spec] allows us to use the fact that a closure [c] satisfies
   the specification predicate [P] to derive that [c] satisfies some
   [P'] such that [P -> P']. *)

Lemma weaken_spec (arg_τ : arg_type) (P P' : arg_τ -#> microvx -> Prop) c :
  Spec c P ->
...
(* [aSpec] is a restatement of [Spec], but with all of the
   arguments quantified at the top.
   We then have the following equivalence:
   [Spec c P <-> ∀# args, aSpec c P args]. *)

(* Equations aSpec (X : Type) (HX : Encode X) (TT : etele) *)
(*   (c : val) (P : (TeleS X TT) -#> microvx -> Prop) *)
(*   : TeleS X TT -#> Prop := *)
(* aSpec X HX TeleO c P := λ (x : X), P x (call c #x) ; *)
(* aSpec X HX (TeleS Y TT) c P := *)
(*   etele_bind (λ (args : (TeleS X (TeleS Y TT))), *)
(*       let x := args.(etele_arg_head X TT) in *)
(*       let args := args.(etele_arg_tail X TT) in *)
(*       pure (call c #x) (λ c, etele_app (aSpec Y _ TT c (P x)) args) ⊥). *)

Equations aSpec_aux (c : val) (args_τ : arg_type)
  (P : args_τ -#> microvx -> Prop) (args : args_τ)
  : Prop :=
  aSpec_aux _ (Arg1 X) P x :=
    P x (call c #x);
  aSpec_aux c (ArgS X args_τ') P {| arg_head := x; arg_tail := args'; |} :=
    pure_wp (call c #x)
      (λ c', aSpec_aux c' args_τ' (P x) args') ⊥
.

Definition aSpec {args_τ : arg_type} (c : val)
  (P : args_τ -#> microvx -> Prop) : args_τ -#> Prop :=
  arg_bind (aSpec_aux c args_τ P).
...
Lemma prove_Spec (arg_τ : arg_type) η x e (P : arg_τ -#> microvx -> Prop) :
...
Section unfold_spec_aux_def.
  ...
  (* However, [prop_rec] exposes nested calls created by [call_tele].
   [build_prop_rec] avoids this by fetching the function body from a
   series of nested [EAnonFun], at a depth equal to the number of
   arguments specified by the telescope. *)
  ...
  | Arg1 X, η, (EAnonFun (AnonFun arg e)), P, R :=
      ∀ (x : X),
      (∀# (sargs : og_arg_τ), R sargs x -> aSpec c Pog sargs) ->
      P x (eval ((arg, #x) :: η) e)
...
Lemma Spec_equiv (arg_τ : arg_type) η f x e (P : arg_τ -#> _) args :
...
Proof with Spec_auto.
  ...
  remember arg_τ as TTog in R at 1, Pog, HP at 1, IHargs; clear HeqTTog.

  (* Generalize the statement for the induction. Notably, generalize
     over the head type of the telescope. *)
  revert x e η0 HP.
  induction arg_τ as [ X HX | X HX arg_τ IH]; intros x e η0 Hbp.

  (* Base case: the telescope is empty. *)
  { cbv... apply Hbp. apply IHargs. }
...
Proof.
  intros. eapply pure_call_equiv. auto.
Qed.

(* [pure_EApp_partial] is a lemma for partial application. Notice that
   the predicate [P] may expects more arguments, which are described
   by [TT]. *)
...
Proof.
  ...
  destruct (HSpec v3) as (v2 & (v1 & HP & Hv1) & Hv2).
  apply pure_call_equiv.
  eapply Hmono; eauto.
Qed.
...
(* [pure_EApp_prop] is the generalisation of [pure_EApp_prop2] and
   [pure_EApp_prop3]. It is defined as a fixpoint, iterating over the
   types of the arguments. For any number of arguments, it generates
   the desired lemma. *)
...
Local Lemma pure_EApp_predetermined1 `{Encode X, Encode Y} η e e2
...
Section pure_EApp_predetermined_def.

  Context (η : env) (ζ : exn -> Prop).
  Context (expr_base : expr) (goal_hyp : expr -> Prop).

  Equations pure_EApp_predetermined (arg_τ : arg_type) (args : arg_τ) (exprs : list expr) : Prop :=
  | Arg1 X, x, exprs :=
      ∀ (e : expr),
  ...
  | ArgS X arg_τ', {| arg_head := x; arg_tail := args' |}, exprs :=
       ∀ (e : expr),
         pure (eval η e) (singleton x) ζ ->
         pure_EApp_predetermined arg_τ' args' (e :: exprs).

End pure_EApp_predetermined_def.

Strategy transparent [ pure_EApp_predetermined ].

Lemma pure_EApp_predetermined_induction_step (arg_τ : arg_type) η e ζ f :
  ∀ args (es : list expr) (ei : expr),
    app_exprs (EApp e ei) es = app_exprs e (es ++ [ei]) ->
    pure_EApp_predetermined η ζ
      (EApp e ei)
      (λ e  : expr, f e)
      arg_τ
      args
...
Lemma pure_EApp_predetermined_mono (arg_τ : arg_type) η e ζ args es (f f' : expr -> Prop) :
  pure_EApp_predetermined η ζ
    e
    (λ e0  : expr, f' e0)
    arg_τ
...
Lemma pure_EApp' `{Encode Y} (arg_τ : arg_type) η e (Ψ : Y -> Prop) ζ (P : arg_τ -#> microvx -> Prop) :
  ∀# (args : arg_τ),
    @pure_EApp_predetermined η ζ
      e
...
Proof with Spec_auto.
  ...
  eapply pure_EApp_predetermined_induction_step.
  { reflexivity. }

  eapply pure_EApp_predetermined_mono.
  { apply IH. }
  intros e0; simpl.
...
Lemma pure_EApp_mono (arg_τ : arg_type) (η : env) ζ e fe es
...
Lemma pure_EApp_prop_induction_step (arg_τ : arg_type) (η : env) e (ζ : exn -> Prop) f :
...
Proof.
  ...
  { (* Subgoal: [ pure (eval η (EApp e ex)) (λ c, Spec c P') ζ ]. *)
    ...
    - intros c' HSpec'; cbn beta in HSpec'.
      unfold returns; eexists; split; [ reflexivity | ].
      eapply weaken_spec; [ apply HSpec' | ].
      apply forallArgs_forall; intros args m HP.
      rewrite args_app_bind.
