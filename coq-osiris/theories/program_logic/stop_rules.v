...
From osiris.program_logic Require Import wp_step ewp tactics basic_rules.
...
Section ewp_stop.
  ...
  Lemma ewp_die o E ι Ψ Φ :
    live_thread ι -∗
    Φ o -∗
    EWP die o @ E <| (ι, Ψ) |> {{ Φ }}.
  Proof.
    iIntros "Hι HΦ".
    iApply (ewp_stop_die with "Hι").
    iApply ewp_outcome2.
    iApply "HΦ".
  Qed.
...
Section ewp_eval.
  ...
  Lemma ewp_module η sitems E Ψ (Q : val -> iProp Σ) :
    EWP (eval_sitems (η, []) sitems) @ E <| Ψ |> {{ RET ηδ, let '(_, δ) := ηδ in Q (VStruct δ) }} -∗
      EWP (eval_mexpr η (MStruct sitems)) @E <| Ψ |> {{ RET v, Q v }}.
  Proof.
    iIntros "Hsitems".
  ...
  Lemma ewp_struct_let η δ bs E Ψ Q Q' :
    EWP (eval_bindings η bs) @ E <| Ψ |> {{ RET η, Q' η }} -∗
