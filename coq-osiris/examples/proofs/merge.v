...
Proof.
  ...
  eapply pure_rec_call with (P := fun _ => True); first done.
  clear l.
  intros split ? IH _.
  ...
  { (* Case: l matches [] *)
    ...
    (* FIXME : Remove [Unshelve.] *)
    Unshelve. 4-5: exact nil.
    all: auto.
    split; simpl; auto. }
  (* Second branch of match *)
  { (* Case: l matches [x] *)
    apply pure_eval_pair. trivial_pure.
    (* Establish postcondition *)
    Unshelve. 5: exact nil. 4: exact [x0].
    all: auto.
    split; simpl; auto. }
  ...
  { (* Case: l matches a::b::t *)
    ...
    apply pure_eval_pair. trivial_pure.
    (* Establish postcondition *)
    Unshelve. (* FIXME : Remove [Unshelve]. *)
    4 : exact (x :: l1). 4 : exact (x0 :: l2).
    { cbn. unfold VCons. repeat f_equiv. }
    { cbn. unfold VCons. repeat f_equiv. }
    cbn. unfold VCons. repeat f_equiv.
    split; [ | split ].
