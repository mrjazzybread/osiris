...
type expr =
  ...
  (* Array access (with or without bounds check):
     [Array.get a n] and [Array.unsafe_get a n] *)
  | EArrayGet of expr * expr
  (* Array update (with or without bounds check):
     [Array.set a n v] and [Array.unsafe_set a n] *)
