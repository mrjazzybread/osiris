...
Goal
  ...
  safe (eval EnvNil e) (λ v, v = VConstant "A").
Proof.
...
Goal safe (eval EnvNil example) (λ v, v = VData "A" (VTuple VNil)).
Proof.
  wp.
...
Goal
  ...
  safe (eval env example2) (λ v, v = v1).
Proof.
  intros. wp.
  wp_continue.
  wp_continue.
...
Lemma spec_example3:
  ...
  safe (eval env example3) (λ v, v = VPair (VConstant "A") (VConstant "A")).
Proof.
  intros id Hid.
  wp.
...
Goal
  safe (eval EnvNil identity) spec_id.
Proof.
...
Lemma spec_example4:
  safe (eval EnvNil example4) (λ v, v = VPair (VConstant "A") (VConstant "A")).
Proof.
  unfold example4.
  wp.
...
Lemma spec_example5:
  safe (eval EnvNil example5) (λ v, v = VFalse).
Proof.
  wp.
...
Lemma spec_example4b:
  safe (eval EnvNil example4b) (λ v, v = VUnit).
Proof.
  unfold example4b. wp.
...
Lemma spec_example4c:
  safe (eval EnvNil example4c) (λ v, v = VUnit).
Proof.
  unfold example4c. wp.
  (* Deal with the local binding of [id]. *)
  wp_specify "id" spec_id.
...
Lemma spec_example4d:
  safe (eval EnvNil example4d) (λ v, v = VUnit).
Proof.
  unfold example4d. wp.
  (* Deal with the local binding of [id]. *)
  wp_specify "id" spec_id.
...
Lemma spec_divergence:
  safe (eval EnvNil divergence) (λ _, False).
Proof.
...
Lemma spec_walk_example_concrete :
  ...
  safe (eval EnvNil (walk_example e)) (λ v, v = encode ()).
Proof.
...
Lemma spec_walk_example_abstract :
  ...
  safe (eval η (walk_example (EVar "xs"))) (λ v, v = encode ()).
Proof.
  intros. wp.
  (* The environment is about to be extended with a binding of the variable
