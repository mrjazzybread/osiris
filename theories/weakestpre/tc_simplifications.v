...
Section TypeclassesDefinitions.
  ...
  (* [TC_change_goal] can either be used to simplify the goal
     (Cf. [inst_TC_change_goal]) or to help with automation (Cf. the section
     [Specifications]. *)
  Class TC_change_goal (P Q: iProp Σ) Δ :=
    tc_change_goal : environments.envs_entails Δ P →
                     environments.envs_entails Δ Q.

  Class TCpsimplify {A} (m m': free A) :=
    tc_psimplify: psimplify m m'.
...
(* The following instances are about simplifying the goal using [psimplify]. *)
Section PSimplify.
  Context `{!osirisGS_gen hlc Σ}.

  Global Instance inst_TC_change_goal {A} s E (m m': free A) φ Δ
         `{p: TCpsimplify A m m'}
    : TC_change_goal (WP m' @ s; E {{ v, φ v }}) (WP m @ s; E {{ v, φ v }}) Δ
    | 200 :=
    tac_change_goal Δ _ _ (wp_psimplify m m' s E φ p).
  Global Hint Mode inst_TC_change_goal + + + + - + + + : typeclass_instances.


  Global Instance inst_TCpsimplify_refl {A} (m: free A)
    : TCpsimplify m m | 1000 := PSimplifyReflexive m.
  Global Hint Mode inst_TCpsimplify_refl + + : typeclass_instances.

  Global Instance inst_TCpsimplify_eval {A}
         (η : env) (e : expr) (k : val → free A) (m' : free A)
    : TCpsimplify (Stop CEval (η, e) k) (' x ← eval η e; k x).
  Proof.
    apply PSimplifyEval, PSimplifyReflexive.
  Qed.
  Global Hint Mode inst_TCpsimplify_eval + + + + + : typeclass_instances.

  Global Instance inst_TCpsimplify_loop {A}
         (η : env) (x : var) (i1 i2 : int) (e : expr) (k : val → free A)
         (m' : free A)
    : TCpsimplify (Stop CLoop (η, x, i1, i2, e) k)
                  (' x ← loop η x i1 i2 e; k x).
  Proof.
    apply PSimplifyLoop, PSimplifyReflexive.
  Qed.
  Global Hint Mode inst_TCpsimplify_loop + + + + + + + + : typeclass_instances.

  Global Instance inst_TCpsimplify_par_ret_ret {A A1 A2}
         (a1 : A1) (a2 : A2) (k : A1 * A2 → free A)
    : TCpsimplify (Par (ret a1) (ret a2) k (λ _ : (), Next)) (k (a1, a2)).
  Proof.
    apply PSimplifyParRetRet, PSimplifyReflexive.
  Qed.
  Global Hint Mode inst_TCpsimplify_par_ret_ret + + + + + +
  : typeclass_instances.

  Global Instance inst_TCpsimplify_par_ret_left {A A1 A2}
         (a1 : A1) (m2 m'2 : free A2) (k : A1 * A2 → free A)
         `{p: TCpsimplify A2 m2 m'2}
    : TCpsimplify (Par (ret a1) m2 k (λ _ : (), Next)) (' v2 ← m'2; k (a1, v2)).
  Proof.
    apply (PSimplifyParRetLeft _ _ _ _ _ p), PSimplifyReflexive.
  Qed.
  Global Hint Mode inst_TCpsimplify_par_ret_left + + + + + - + -
  : typeclass_instances.

  Global Instance inst_TCpsimplify_par_ret_right {A A1 A2}
         (m1 m'1 : free A1) (a2 : A2) (k : A1 * A2 → free A)
         `{p: TCpsimplify A1 m1 m'1}
    : TCpsimplify (Par m1 (ret a2) k (λ _ : (), Next)) (' v1 ← m'1; k (v1, a2)).
  Proof.
    apply (PSimplifyParRetRight _ _ _ _ _ p), PSimplifyReflexive.
  Qed.
  Global Hint Mode inst_TCpsimplify_par_ret_right + + + + - + + -
  : typeclass_instances.

  Global Instance inst_TCpsimplify_par {A}
         (A1 A2 : Type) (m1 m'1 : free A1) (m2 m'2 : free A2) (k : A1 * A2 → free A)
         (ko : () → free A)
         `{p: TCpsimplify A1 m1 m'1}
         `{p': TCpsimplify A2 m2 m'2}
    : TCpsimplify (Par m1 m2 k ko) (Par m'1 m'2 k ko) | 150 :=
    SimplifyPar _ _ _ _ _ _ _ p p' (PSimplifyReflexive _).
  Global Hint Mode inst_TCpsimplify_par + + + + - + - + + - -
  : typeclass_instances.
