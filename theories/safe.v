...
Lemma safe_refinement_par {A1 A2} (m1 m1' : free A1) (m2 m2' : free A2)
  (φ : A1 * A2 → Prop) :
  refinement _ m1 m1'
  -> refinement _ m2 m2'
  -> safe (par m1' m2') φ
  -> safe (par m1 m2) φ.
Proof.
  intros H1 H2 (φ1 & φ2 & Hm1' & Hm2 & H)%invert_safe_par.
  rewrite safe_par'.
  exists φ1, φ2.
  repeat split.
  - now apply H1.
...
Proof.
  ...
  eapply safe_refinement_par; eauto.
  rewrite safe_par'.
  exists φ1, φ2.
  repeat split; auto.
  intros v1 v2 H1 H2.
...
#[global]
Instance refinement_proper_Par A B C:
  Proper (refinement A ==> refinement B ==> eq ==> refinement C)
         (fun e e' φ => Par e e' φ (λ _, Next)).
Proof.
  intros a1 a2 Ha b1 b2 Hb ? k -> φ' H2.
  pose proof (invert_safe_par a2 b2 k φ' H2) as (φ1 & φ2 & Ha2 & Hb2 & H2').
...
Lemma call_ret_ret e e' v v':
  refinement _ e (ret v)
  -> refinement _ e' (ret v')
  -> refinement _
    (Par' e e' (λ '(v1, v2), call v1 v2))
    (call v v').
Proof.
  intros He He' φ H.
  apply safe_par.
